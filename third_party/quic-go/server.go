...
func (s *baseServer) handleInitialImpl(p receivedPacket, hdr *wire.Header) error {
	...
	if err != nil {
		// ConnContext may already have reserved application capacity. No
		// connection object exists yet to own cancellation or the packet buffer,
		// so unwind both explicitly on this rare random-source failure.
		cancel(err)
		if qlogTrace != nil {
			_ = qlogTrace.AddProducer().Close()
		}
		delete(s.zeroRTTQueues, hdr.DestConnectionID)
		p.buffer.Release()
		return err
