...
func newTestServer(t *testing.T, serverOpts *serverOpts) *testServer {
	t.Helper()
	c, err := wrapConn(newUDPConnLocalhost(t), false)
...
type failingConnectionIDGenerator struct{ err error }

func (g failingConnectionIDGenerator) GenerateConnectionID() (ConnectionID, error) {
	return ConnectionID{}, g.err
}

func (failingConnectionIDGenerator) ConnectionIDLen() int { return 8 }

func TestServerCancelsConnContextWhenConnectionIDGenerationFails(t *testing.T) {
	want := errors.New("connection ID randomness failed")
	server := newTestServer(t, &serverOpts{})
	server.connIDGenerator = failingConnectionIDGenerator{err: want}
	canceled := make(chan error, 1)
	server.connContext = func(ctx context.Context, _ *ClientInfo) (context.Context, error) {
		derived, cancel := context.WithCancelCause(ctx)
		go func() {
			<-derived.Done()
			canceled <- context.Cause(derived)
		}()
		_ = cancel // cancellation is owned by the server's derived context
		return derived, nil
	}
	...
	case <-time.After(time.Second):
		t.Fatal("ConnContext was not canceled after connection ID generation failed")
	}
}
