...
func TestStreamAdmissionRunsBeforeFramePeekAndReleases(t *testing.T) {
	clientConn, serverConn := newConnPair(t)
	admitted := make(chan struct{})
	released := make(chan struct{})
	dispatched := make(chan error, 1)
	server := &Server{
		StreamAdmission: func(stream *quic.Stream) (func(), bool) {
			close(admitted)
			require.NoError(t, stream.SetReadDeadline(time.Now().Add(20*time.Millisecond)))
			return func() { close(released) }, true
		},
		StreamDispatcher: func(_ FrameType, _ *quic.Stream, err error) (bool, error) {
			dispatched <- err
			return true, nil
		},
	}
	...
	require.NoError(t, err)
	defer stream.Close()
	// A QUIC stream isn't visible to the peer until it carries data. Send only
	// the first byte of a two-byte varint so AcceptStream returns while the
	// frame-type peek itself remains incomplete.
	_, err = stream.Write([]byte{0x40})
	require.NoError(t, err)
	select {
	case <-admitted:
		// Reaching admission while the frame varint is incomplete proves it
		// precedes quicvarint.Peek.
	case <-time.After(time.Second):
		t.Fatal("stream was not admitted before the frame peek")
	}
	select {
	case err := <-dispatched:
		require.Error(t, err)
	case <-time.After(time.Second):
		t.Fatal("pre-peek read deadline did not interrupt dispatch")
	}
	select {
	case <-released:
	case <-time.After(time.Second):
		t.Fatal("admission release callback was not called")
	}
}

func TestStreamAdmissionHoldsSlotUntilDispatcherReturns(t *testing.T) {
	clientConn, serverConn := newConnPair(t)
	slot := make(chan struct{}, 1)
	dispatchStarted := make(chan struct{})
	unblockDispatch := make(chan struct{})
	rejected := make(chan struct{})
	released := make(chan struct{})
	server := &Server{
		StreamAdmission: func(_ *quic.Stream) (func(), bool) {
			select {
			case slot <- struct{}{}:
				return func() {
					<-slot
		...
		StreamDispatcher: func(_ FrameType, _ *quic.Stream, _ error) (bool, error) {
	...
	first, err := clientConn.OpenStream()
	require.NoError(t, err)
	defer first.Close()
	_, err = first.Write(quicvarint.Append(nil, 0x21))
	require.NoError(t, err)
	select {
	case <-dispatchStarted:
	case <-time.After(time.Second):
		t.Fatal("first stream was not dispatched")
	}

	second, err := clientConn.OpenStream()
	require.NoError(t, err)
	defer second.Close()
	_, _ = second.Write(quicvarint.Append(nil, 0x21))
	select {
	case <-rejected:
	case <-time.After(time.Second):
		t.Fatal("second stream bypassed the occupied admission slot")
	}
	select {
	case <-released:
		t.Fatal("slot was released while the dispatcher was still running")
	default:
	}

	close(unblockDispatch)
	select {
	case <-released:
	case <-time.After(time.Second):
		t.Fatal("slot was not released after the dispatcher returned")
	}
}
