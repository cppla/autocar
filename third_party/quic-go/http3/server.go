...
type Server struct {
	...
	// StreamAdmission, when set, runs synchronously immediately after accepting
	// a bidirectional stream and before starting a handler goroutine or reading
	// the first frame byte. Returning false resets the stream without spawning a
	// handler. The returned release function, when non-nil, is called exactly
	// once after dispatch or HTTP request handling completes.
	//
	// This AutoCAR extension allows a caller to apply process-wide admission and
	// a first-byte deadline before StreamDispatcher's frame-type peek.
	StreamAdmission func(*quic.Stream) (release func(), admitted bool)
...
func (s *Server) handleConn(conn *quic.Conn) error {
	...
	for {
		...
		release := func() {}
		if s.StreamAdmission != nil {
			var admitted bool
			release, admitted = s.StreamAdmission(str)
			if !admitted {
